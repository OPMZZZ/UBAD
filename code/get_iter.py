...
def get_scores(trainer, batch):
    ...
    with torch.no_grad():
        ...
        err = (err_map4 + err_map3+ err_map1 + err_map2).mean(dim=1, keepdim=True)
...
def get_img(trainer, dataset, batch_size=1, get_y=lambda batch: batch[1], threshold=None):
    ...
    def save_nii(volume, fname, pname):
        for index in range(volume.shape[1]):
            ...
            nib.save(real_volume, f'../outputs/{dataset_name}/iter/{pname}/{fname}-{index}.nii.gz')
    ...
    with torch.no_grad():
        ...
        for pd in dataset.patient_datasets:
            ...
            batch_y = get_y(batch)
            batch = batch[0]
            reconstructions = batch[:, trainer.input_channel]
            for iter in range(100):
                anomaly_scores, reconstructions, rnm = get_scores(trainer, reconstructions)
                anomaly_scores = median_filter_3D(anomaly_scores.permute(1, 2, 3, 0), kernelsize=5).permute(3, 0, 1, 2)


                os.makedirs(f'../outputs/{dataset_name}/iter/{name}/{iter}', exist_ok=True)


                anomaly_scores_ = anomaly_scores.cpu()


                save_nii(batch[:, trainer.input_channel].numpy(), 'real_volume', f'{name}/{iter}')
                save_nii(batch_y.numpy(), 'mask', f'{name}/{iter}')
                save_nii(reconstructions.numpy(), 'reconstructions', f'{name}/{iter}')
                save_nii(rnm.numpy(), 'rnm', f'{name}/{iter}')
                save_nii(anomaly_scores_.numpy(), 'anomaly_scores', f'{name}/{iter}')
...
def evaluate(split: str = "test", use_cc: bool = True):
    ...
    results = get_img(trainer, dataset=dataset)
