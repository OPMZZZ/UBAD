...
def process_patient(path, target_path):
    flair = nib.load(path / "ses-0001" / 'anat'/ f"{path.name}_ses-0001_FLAIR.nii.gz").get_fdata()
    dwi = nib.load(path / "ses-0001" / 'anat'/ f"{path.name}_ses-0001_dwi.nii.gz").get_fdata()
    adc = nib.load(path / "ses-0001" / 'anat'/ f"{path.name}_ses-0001_adc.nii.gz").get_fdata()
    fusion = (flair + dwi + adc) / 3
    labels = nib.load(path / "ses-0001" / 'anat'/ f"{path.name}_ses-0001_msk.nii.gz").get_fdata()

    volume = torch.stack([torch.from_numpy(x) for x in [flair, dwi, adc, fusion]], dim=0).unsqueeze(dim=0)
...
def preprocess(datapath: Path):
    ...
    splits_path = Path(__file__).parent.parent / "data" / "isle_preprocessed" / "data_splits"
    ...
    for split in ["train", "val", "test"]:
        ...
        for source_path in tqdm(paths):
            target_path = Path(__file__).parent.parent / "data" / "isle_preprocessed" / f"npy_{split}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", type=str, default="../../ISLE/"
