...
def process_patient(path, target_path):
    ...
    T1 = nib.load(root_path / "t1" / f"{prefix}_T1w.nii.gz_t1.nii.gz").get_fdata()

    labels = nib.load(root_path / "seg" / f"{prefix}_seg.nii.gz").get_fdata()
    ...
    for slice_idx in range(fs_dim2, ls_dim2):
        ...
        low_res_y = torch.flip(low_res_y, dims=[-2, -1])
...
def preprocess(datapath: Path):
    ...
    splits_path = Path(__file__).parent.parent / "data" / "ATLAS_preprocessed" / "data_splits"
    ...
    for split in ["train", "val", "test"]:
        ...
        for source_path in tqdm(paths):
            target_path = Path(__file__).parent.parent / "data" / "ATLAS_preprocessed" / f"npy_{split}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", type=str, default="E:/1/data/ATLAS/t1"
