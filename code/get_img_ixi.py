...
def get_img(trainer, dataset, batch_size=1, get_y=lambda batch: batch[1], threshold=None):
    ...
    with torch.no_grad():
        ...
        for pd in dataset.patient_datasets:
            ...
            save_nii(batch[:, [0]].numpy(), 'real_volume', name)
...
def evaluate(split: str = "test", use_cc: bool = True):
    dataset = 'MSLUB'
    val_dataset = BrainDataset(dataset=dataset, split='test', modalities='t2')
    res = []
    for j in [ 1]:
        ...
        results = get_img(trainer, dataset=val_dataset, threshold=threshold)
        res.append(results)
    return results
