...
from cross_data_descriptor import DataDescriptor as CrossDataDescriptor
...
class Trainer:
    ...
    def load(self, identifier=None, load_optimiser=True, exclude_keys=[], dir="saved_models"):
        ...
        # 在 test_trainer.py 文件的 load 函数中
        ...
        if "epoch_no" in checkpoint:
            ...
            print(f"Loaded epoch no: {self.state['epoch_no']}")
        if "train_it" in checkpoint:
            self.state["train_it"] = checkpoint["train_it"]
            print(f"Loaded train iterations: {self.state['train_it']}")
        return self
    ...
    def set_data(self, data: Optional[DataDescriptor]):
        ...
        if isinstance(data, DataDescriptor) or isinstance(data, CrossDataDescriptor):
            dd = data
        elif data is None:
